...
public class UpdateFeeDetails extends javax.swing.JFrame {
    ...
    public boolean validation()
    {
        ...
        if (txt_courseAmount.getText().equals("")) {
    ...
    public void fillComboBox()
    {
     ...
     public String updateData(){
         ...
         try {
             Connection con=DBconnection.getConnection();
             PreparedStatement pst=con.prepareStatement("update fees_details set student_name= ?,roll_No= ?,payment_mode= ?,cheque_no= ?,"
                     + "bank_name= ? ,dd_no= ? ,courses= ? ,gstin= ? ,total_amount= ? ,date= ? ,amount= ? ,cgst= ? ,sgst= ? ,total_in_words= ? ,remark= ? ,year1= ? ,year2= ? "
                     + "where reciept_no = ? ");
     ...
     public void setRecords(){
         try {
            ...
            txt_totalAmout.setText(rs.getString("total_amount"));
    ...
    private void btn_printActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_printActionPerformed
        ...
        if (result.equals("Success")) {
            JOptionPane.showMessageDialog(this,"record updated succussful");
